...
# Create the schema up front so tests that touch the DB WITHOUT starting the app
# (e.g. oauth_flow unit tests, whose promotion guard reads the committed server row)
# work in any subset/order. Normally app startup does this; a standalone run of one
# test file would otherwise hit "no such table". Idempotent against the throwaway
# (or explicitly pinned) data dir.
init_db()
